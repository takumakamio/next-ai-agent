...
import { ManageQaForm } from '@/features/root/qas/components/form'
import { ManageQaTable } from '@/features/root/qas/components/table'
...
export const QasTab = () => {
  ...
  const handleDelete = async (id: string) => {
    try {
      await clientDelete(rpc.api.root.qas[':id'], { param: { id } })
  ...
  const handleEditClick = async (id: string) => {
    try {
      const qa = await clientFetch(rpc.api.root.qas[':id'], { param: { id } })
      setEditingQa(qa)
      setView('form')
    } catch (error) {
